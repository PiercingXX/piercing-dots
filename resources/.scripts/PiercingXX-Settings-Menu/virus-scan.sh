...
service_try() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        local svc
        for svc in "$@"; do
            sudo systemctl "$action" "$svc" 2>/dev/null && return 0
        done
    fi
    ...
    if command -v sv >/dev/null 2>&1; then
        local svc
        for svc in "$@"; do
            sudo sv "$action" "$svc" 2>/dev/null && return 0
        done
    fi
...
start_clamd_if_available() {
    ...
    service_try start clamav-daemon clamd@scan clamd >/dev/null 2>&1 || true
}
...
update_definitions() {
    ...
    service_try stop clamav-freshclam.service clamav-freshclam freshclam >/dev/null 2>&1 || true
    ...
    if [ $? -eq 0 ]; then
        ...
        service_try start clamav-freshclam.service clamav-freshclam freshclam >/dev/null 2>&1 || true
    else
