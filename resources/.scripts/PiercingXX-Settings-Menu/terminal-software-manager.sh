...
install_software() {
    ...
    case "$dtype" in
        ...
        "void")
            (
                xbps-query -Rs '' 2>/dev/null | awk '{print $2}' | sed -E 's/-[0-9][^ ]*$//' | grep -v '^$' | sort -u | sed 's/^/[repo] /'
                if command -v flatpak &>/dev/null; then
                    flatpak remote-ls --app flathub --columns=application 2>/dev/null | grep -v '^$' | LC_ALL=C grep -v '[^[:print:]]' | sed 's/^/[flatpak] /'
                fi
            ) | fzf --multi \
                --preview='name=$(echo {} | cut -d" " -f2-); if echo {} | grep -q "^\[flatpak\]"; then flatpak remote-info flathub "$name" 2>/dev/null || echo ""; else xbps-query -RS "$name" 2>/dev/null || echo ""; fi' \
                --preview-window=down:50% \
            | xargs -ro -I{} bash -c '
                sel="{}"
                name=$(printf "%s" "$sel" | cut -d" " -f2-)
...
uninstall_software() {
    ...
    case "$dtype" in
        ...
        fedora)
            ...
            ;;
        void)
            native_list_cmd="xbps-query -l | awk '{print \$2}' | sed -E 's/-[0-9][^ ]*$//'"
            ;;
        *)
