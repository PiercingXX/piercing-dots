...
一个技能,用于生成**单文件 HTML 横向翻页 PPT**,视觉基调是"**电子杂志 × 电子墨水**"——像 *Monocle* 贴上了代码的样子。
...
> 帮我安装 `guizang-ppt-skill` 这个 skill。请按下面步骤做:
>
> 1. 确保 agent 的 skills 目录存在(不存在就创建)
> 2. 执行 `git clone https://example.com/skills/magazine-web-ppt.git` 到该 skills 目录
> 3. 验证克隆里有 `SKILL.md`、`assets/`、`references/` 三项
> 4. 告诉我安装好了,之后我说"做一份杂志风 PPT"之类的话就会触发这个 skill

把这段话复制粘贴给任何有 shell 权限的 AI Agent,它会自动完成安装。
