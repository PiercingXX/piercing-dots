...
def load_api_key():
    ...
    if key:
        return key
    key_file = os.path.expanduser("~/.config/openweather/api_key")
    try:
...
if __name__ == "__main__":
    if not api_key:
        print("weather.py: no API key; set OPENWEATHER_API_KEY or ~/.config/openweather/api_key", file=sys.stderr)
        sys.exit(0)
    while True:
